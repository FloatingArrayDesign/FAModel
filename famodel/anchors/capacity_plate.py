...
def getCapacityPlate(A, B_t_aspect=40, Hs=20, Beta=30, Los=0.05, 
    ...
    t = round(B/B_t_aspect,2)  # thickness of the plate, which it depends on the width [m]
    
    #t=np.sqrt(0.006*A)/4  
    V = round(A*t,2)           # steel volume [m3]
    W = V*St_density           # plate weight [kg]
    #Su0 = 10
    Suh = So0 + k*Hs           # The final clay undrained shear strength
...
if __name__ == '__main__':
    ...
    print('Anchor steel volume,        ' , results['vol'], '[m3]') 
