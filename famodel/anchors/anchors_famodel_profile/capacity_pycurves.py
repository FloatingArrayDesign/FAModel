...
def py_Matlock(z, D, zlug, f_Su, f_sigma_v_eff, f_gamma, z0=None, plot=True):
    ...
    plot : bool
        Plot the resulting p–y curve if True
...
def py_API(z, D, zlug, f_phi, f_sigma_v_eff, f_Dr, z0=None, plot=True):   
...
def py_Reese(z, D, zlug, f_UCS, f_Em, z0=None, plot=True):
...
def py_Lovera(z, D, f_UCS, f_Em, zlug, z0, plot=True):
    ...
    if plot:
        ...
        plt.xlabel('y (m)')
        plt.ylabel('p (N/m)')
        plt.title('PY Curves - Lovera (2019)')
        plt.grid(True)
        plt.xlim([-0.1*D, 0.1*D])
        plt.ylim([min(p), max(p)]) 
        plt.show()
