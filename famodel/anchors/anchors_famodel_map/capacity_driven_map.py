...
from .capacity_solvers import fd_solver
from .capacity_pycurves_map import py_Matlock, py_API, py_Reese
from .capacity_plots_map import plot_pile, plot_pycurve
...
def getCapacityDriven(profile_map, location_name, D, L, zlug, Ha, Va, plot=True):
    ...
    for i in range(2, n+3):
        ...
        if soil_type == 'clay':
            ...
            py_f, (y_vals, p_vals) = py_Matlock(z_depth, D, zlug, f_Su, f_sigma_v_eff, f_gamma, z0=z0_local, return_curve=True)
            py_funs.append(py_f)
            ...
            py_f, (y_vals, p_vals) = py_API(z_depth, D, zlug, f_phi, f_sigma_v_eff, f_Dr, z0=z0_local, return_curve=True)
            py_funs.append(py_f)
            ...
            UCS = f_UCS(z_depth)
            Em = f_Em(z_depth)
            py_f, (y_vals, p_vals) = py_Reese(z_depth, D, zlug, f_UCS, f_Em, z0=z0_local, return_curve=True)
            py_funs.append(py_f)
