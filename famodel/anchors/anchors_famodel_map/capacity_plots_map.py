...
def plot_load(layers, drag_values, depth_values, Tm, thetam, Ta, thetaa, zlug):
    ...
    ax.axhline(-layers[0]['top'], color='k', linestyle='--', lw=1.5, label=f'Mudline')

    # Annotate loads
    ax.annotate(f"{Tm/1e6:.2f} MN", (Tm*np.cos(np.deg2rad(thetam))/scale, 
                                     -layers[0]['top'] + Tm*np.sin(np.deg2rad(thetam))/scale), color='r')
    ax.annotate(f"{Ta/1e6:.2f} MN", (drag_values[-1] + Ta*np.cos(np.deg2rad(thetaa))/scale,
                                     depth_values[-1] + Ta*np.sin(np.deg2rad(thetaa))/scale), color='g')
    ...
    ax.grid(True)
    #ax.set_ylim(min(zlug - 10, min(depth_values) - 5), max(15, max(depth_values) + 5))
