...
from .capacity_soils_map import rock_profile
from .capacity_solvers import fd_solver
from .capacity_pycurves_map import py_Lovera
...
def getCapacityDandG(profile_map, location_name, L, D, zlug, Ha, Va, plot=True):
    ...
    for i in range(2, n+3):       # Real nodes
        ...
        UCS = f_UCS(z_depth)
        Em = f_Em(z_depth)
        py_f, (y_vals, p_vals) = py_Lovera(z_depth, D, f_UCS, f_Em, zlug, z0, return_curve=True)
        py_funs.append(py_f)
