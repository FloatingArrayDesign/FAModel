...
def py_Matlock(z, D, zlug, f_Su, f_sigma_v_eff, f_gamma, z0=None, return_curve=False):
   ''' Generate Matlock (1970) p–y curve at a given depth in clay.
    ...
    D : float
        Pile diameter (m)
    zlug : float
        Load eccentricity above or below mudline (m)
    f_Su : function
   ...
   f = interp1d(y, p, kind='linear', bounds_error=False, fill_value=0.0)   # Interpolation function for p-y curve

   return (f, (y, p)) if return_curve else f 
       
def py_API(z, D, zlug, f_phi, f_sigma_v_eff, f_Dr, z0=None, return_curve=False):   
...
def py_Reese(z, D, zlug, f_UCS, f_Em, z0=None, return_curve=False):
...
def py_Lovera(z, D, f_UCS, f_Em, zlug, z0, delta_grout=0.075, E_grout=20e9, delta_crushed=0.025, return_curve=False):
